...
func (s *Solver) createCapMonsterTask(taskData string) (uint64, error) {
	if s.apiKey == "" {
		return 0, errors.New("apiKey is not set")
	}
	...
	request, err := http.Post("https://api.capmonster.cloud/createTask", "application/json", bytes.NewBuffer(data))
	if err != nil {
		return 0, err
	}
	defer request.Body.Close()
	...
	if response.Error != 0 {
		return 0, fmt.Errorf("CapMonster error: %d", response.Error)
	}

	return response.Task, nil
}

func (s *Solver) getCapMonsterTaskResult(task uint64) (string, error) {
	for i := 0; i < 12; i++ {
		<-time.After(time.Second * 10)
		...
		request, err := http.Post("https://api.capmonster.cloud/getTaskResult", "application/json", bytes.NewBuffer(data))
		if err != nil {
			return "", err
		}
